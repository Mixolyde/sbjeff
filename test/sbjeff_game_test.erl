...
unit_test() ->
  ...
  4 = length(Players),
  {error, badarg, []} = sbjeff_game:new_session([]),
  {OnePlayer, ThreePlayers} = lists:split(1, Players),
  {error, badarg, OnePlayer} = sbjeff_game:new_session(OnePlayer),
  {error, badarg, thing} = sbjeff_game:new_session(thing),
  FivePlayers = Players ++ OnePlayer,
  {error, badarg, FivePlayers} = sbjeff_game:new_session(FivePlayers),
